	...
	$('input:radio').on('ifChecked', function(event){
		if($(this).hasClass("open-fs")){
			if($(this).hasClass("open-first")){
				$(this).closest('.form-group').next().removeClass('hide')
				$(this).closest('.form-group').next().next().addClass('hide')
			}else if($(this).hasClass("open-second")){
				$(this).closest('.form-group').next().next().removeClass('hide')
				$(this).closest('.form-group').next().addClass('hide')
			}
		}else{
			if($(this).hasClass("authType")){
				...
				if($(this).attr("id")=="sso"){
					$(this).closest('.form-group').next().next().next().removeClass('hide');
				}
			}
			else{
				if($(this).hasClass("open-yn")){
					if($(this).hasClass("open")){
					$(this).closest('.form-group').next('.specific-questions').removeClass('hide');
			   
					}else{
						$(this).closest('.form-group').next('.specific-questions').addClass('hide');
					}
				}
			}
...
function getHostInfo()	{
	...
	$.ajax({
		...
		success : function(data) {
			...
			var result = "Host Info<br /> Domain : " + data.domain + "<br /> IP Address : " + data.ip;
			  $.getJSON("http://ipinfo.io/"+data.ip, function (response)
               {
	...
	res.forEach(function(r) {
		...
		if (r.vulnerabilities && r.vulnerabilities.length > 0) {
			...
			r.vulnerabilities.forEach(function(v) {
				...
				//? v.identifiers.mapOwnProperty(function(val) { return val }).flatten().join(" ") : " ";
				var info = td(tr);
